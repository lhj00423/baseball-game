...
function swing(){
    ...
    if( swing <0.5){ //50퍼 확률로 타격
        hit();
        strike_count = 0;
        ball_count = 0;
    }else if(swing<0.6){ //10퍼 확률로 번트
...
//파울 함수
function foul(){ 
    if(strike_count == 2){ //스트라이크2일경우 그대로 유지
        strike_count ==2
    }else{
...
function strikecount(){
    ...
    if (strike_count ==1){ //1스트라이크시  불등장
        scount.append(scountcolor)
    }
    if (strike_count ==2){ //2스트라이크시 두번째 불등장
...
function change(){ 
    ...
    if(out_count%3 == 0){ //아웃카운터가3이면 버튼 변경 볼,스트라이크 > 스윙 웨이트
        ...
        score = 0;
        com = !com;
        if(gamecount == 18){ //게임카운터가 18이되면 버튼 삭제
